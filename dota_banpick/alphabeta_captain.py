...
def alphabeta(node: StateNodeCaptain, depth, alpha, beta, is_maximizing_player, depth_limit, activate_saving_cache=ACTIVATE_SAVING_CACHE, cache_dict=None):
    ...
    # * Special case, when cur round = 0, we want to consider ban hero counter
    # TODO this is where we can put heuristic, e.g., 战队擅长什么，对手擅长什么

    # * Terminal condition
    if depth > depth_limit or node.is_terminated():
        value = calculate_heuristic_captain(
            node.get_ally_hero_list(), node.get_opponent_hero_list(), node.get_ban_hero_list(), node.cur_round)
