...
def get_position_colour_tags():
    output = ["#A5BE8F", "#3D87BB", "#DAB77F", "#DE5730", "#EC9DC3"]
    return output
...
def load_default_hero_pools():
    hero_pool_fps = glob(os.path.join(record_folder, "default_pos_*_hero_pool.txt"))
...
def load_today_hero_winrate_dict():
    today_hero_winrate_dict_fp = os.path.join(
        record_folder, "hero_winrate_info_dict.pkl"
    )
...
def load_winlane_hero_dict():
    hero_lanewin_versus_dict_fp = os.path.join(
        record_folder, "hero_lanewin_versus_dict.pkl"
    )
    with open(hero_lanewin_versus_dict_fp, "rb") as f:
        hero_lanewin_versus_dict = pickle.load(f)
    hero_lanewin_with_dict_fp = os.path.join(
        record_folder, "hero_lanewin_with_dict.pkl"
    )
...
@st.cache_data
def load_lane_rate_info_dict():
    lane_info_dict_fp = os.path.join(record_folder, "lane_rate_info_dict.pkl")
    with open(lane_info_dict_fp, "rb") as f:
        lane_info_dict = pickle.load(f)
...
def load_alpha_beta_cache_dict():
    ...
    if os.path.exists(
        os.path.join(record_folder, "depth_limit_1_warmup_cache_dict.pkl")
    ):
        with open(
            os.path.join(record_folder, "depth_limit_1_warmup_cache_dict.pkl"), "rb"
        ) as f:
            d = pickle.load(f)
...
def render_image_url_table(table, image_columns):
    image_col_config = {
        column_name: st.column_config.ImageColumn(column_name)
...
def hero_counter_display_table(title, pos_ind, table):
    annotated_text(
        "Recommended heroes if you play ",
        (
            pos_description[pos_ind].split(" ")[-1],
            f"pos {pos_ind + 1}",
            get_position_colour_tags()[pos_ind],
        ),
    )
    render_image_url_table(table, ["CNTR Pick", "Good With", "Avoid Pick"])
