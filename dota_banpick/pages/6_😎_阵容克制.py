...
def hero_pick_selectbox_to_counter_on_change():
    ...
    with_dict, counter_dict, bad_counter_dict = (
        compute_with_and_counter_heroes_for_each_pos(heronames, suggest_num)
    )
...
if __name__ == "__main__":
    ...
    if "p4_target_hero_with_dict" in st.session_state:
        ...
        st.info(
            f"请根据你的位置, 查看推荐英雄, 如果{heroname}是你的队友在玩, 你可以选择Good With一栏的英雄配合{heroname}, 如果{heroname}是敌人在玩, 你可以选择CNTR Pick一栏的英雄克制{heroname}，避免选择Avoid Pick一栏的英雄, 这些英雄会被{heroname}克制。"
        )
        ...
        for col_ind in range(5):
            ...
            if st.session_state["player_pos_multiselect"] != []:
                if str(position) not in st.session_state["player_pos_multiselect"]:
                    continue
            dataframe = {
                "CNTR Pick": get_online_image_urls(
                    st.session_state["p4_target_hero_counter_dict"][position]
                ),
                "Good With": get_online_image_urls(
                    st.session_state["p4_target_hero_with_dict"][position]
                ),
                "Avoid Pick": get_online_image_urls(
                    st.session_state["p4_target_hero_bad_counter_dict"][position]
                ),
            }
        ...
        for pos_info_i, the_hero in enumerate(st.session_state["p4_target_hero"]):
            with pos_cols[pos_info_i]:
                st.image(
                    get_online_image_urls(
                        [st.session_state["p4_target_hero"][pos_info_i]]
                    ),
                    width=100,
                    caption=st.session_state["p4_target_hero"][pos_info_i],
                )
