...
[data-testid="column"] {
    ...
    if "ally_player_names" not in st.session_state:
        st.session_state["ally_player_names"] = [
            "Pos 1 Bot",
            "Pos 2 Bot",
            "Pos 3 Bot",
            "Pos 4 Bot",
    ...
    if st.session_state["activate_basic_info_flag"]:
        ...
        uploaded_file = st.file_uploader(
            "You can also upload your hero pools file",
            type=["json"],
            key=f"uploader_key",
            on_change=file_upload_onchange,
        )

        with st.form("player_basic_info"):
            pos_multi = st.multiselect(
                "Player Position (if you can play multi-pos, or you want to submit your friends' hero pool, re-edit in the next round)",
                pos_description,
                key="form_pos_multi",
                default=pos_description[-1],
            )
    ...
    if st.session_state["activate_hero_flag"]:  # add new content
        st.info(
            f"Hi {st.session_state['target_player_name']}, please adjust your hero pool."
        )
        ...
        with st.form("activate_hero_info"):
            for t in ["Strength", "Agility", "Intelligence", "Universal"]:
                with st.expander(f"{t}", expanded=True):
                    ...
                    df["If Reco"] = (
                        df["Name"]
                        .str.strip()
                        .isin(st.session_state["target_hero_pool"])
                    )
            ...
            st.form_submit_button(
                "Complete Your Edit", on_click=complete_edit_callback, type="primary"
            )
    ...
    if st.session_state["display_stats_flag"]:
        ...
        with cols[0]:
            st.download_button(
                "Download Your Hero Pool Json File",
                download_json,
                f"hero_pool_id_{st.session_state.data_ready_count}.json",
                key=f"dldbtn_{st.session_state.data_ready_count}",
                use_container_width=True,
                type="primary",
            )
        with cols[1]:
            edit_again_but = st.button(
                "Add/Edit Hero Pool Again", use_container_width=True, type="primary"
            )
            if edit_again_but:
                st.session_state["activate_hero_flag"] = False
                st.session_state["display_stats_flag"] = False
        ...
        for k in cache_dict:
            with st.expander(k + "'s hero pool", expanded=True):
                ...
                delete_but = st.button(
                    "Delete Data",
                    key=f"delete_but_{k}",
                    on_click=delete_cache_player_data,
                    args=(k,),
                )
