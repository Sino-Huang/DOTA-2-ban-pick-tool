...
from dota_banpick.heuristic import (
    compute_associated_opposite_suggestion_first_round,
    compute_bad_picks_for_each_pos,
)
...
def update_ban_hero_multiselect():
    ...
    if st.session_state.the_bp_node.ally_heros.count(None) != 0:
        ...
        if "Preparation Drafting" in st.session_state.suggest_header_placeholder:
            ...
            prepare_phase_suggested_ban_dict = (
                compute_associated_opposite_suggestion_first_round(
            ...
            for ind, comboname in enumerate(prepara_phase_suggested_pick_dict):
                ...
                if len(impacted_player_lst) > 0:
                    st.session_state[f"suggest_ban_table_col_{ind}_table_header"] = (
                        f"Pos Combo {comboname} for {' and '.join(impacted_player_lst)}"
                    )
                else:
                    st.session_state[f"suggest_ban_table_col_{ind}_table_header"] = (
                        f"Pos Combo {comboname}"
                    )

                pick_list = [x for x, _ in prepara_phase_suggested_pick_dict[comboname]]
                pick_list, pick_list_inds = get_diverse_suggest_lst(
                    pick_list, suggest_num
                )

                ban_list = [x for x in prepare_phase_suggested_ban_dict[comboname]]
                ban_list = [ban_list[x] for x in pick_list_inds]
                pick_ban_table = form_pick_ban_table(pick_list, ban_list, comboname)
            ...
            for ind, comboname in enumerate(prepara_phase_suggested_pick_dict):
                ...
                pick_avoid_table = form_pick_avoid_table(pick_list, comboname)
        ...
        if nonecount in [3, 1]:
            ...
            if nonecount_before != nonecount:
                ...
                for ind, comboname in enumerate(prepara_phase_suggested_pick_dict):
                    ...
                    if len(impacted_player_lst) > 0:
                        st.session_state[
                            f"suggest_ban_table_col_{ind}_table_header"
                        ] = f"Pos Combo {comboname} for {' and '.join(impacted_player_lst)}"
                    else:
                        st.session_state[
                            f"suggest_ban_table_col_{ind}_table_header"
                    ...
                    pick_sugg_table = form_pick_avoid_table(pick_list, comboname)
                    st.session_state[f"suggest_ban_table_col_{ind}_table"] = (
                        pick_sugg_table
                    )
                ...
                while f"suggest_ban_table_col_{ind + 1}_table" in st.session_state:
                    ...
                    del st.session_state[
                        f"suggest_ban_table_col_{ind + 1}_table_header"
                    ]
                    ind += 1
...
def ally_pick_select(select_key, ally_ind):
    ...
    if st.session_state.the_bp_node.ally_heros.count(None) in [4, 2]:
        ...
        for ind, comboname in enumerate(prepara_phase_suggested_pick_dict):
            ...
            pick_list = [x for x, _ in prepara_phase_suggested_pick_dict[comboname]]
            pick_list, pick_list_inds = get_diverse_suggest_lst(
                pick_list, suggest_num, suggest_num
            )
            if prepare_phase_suggested_ban_dict is not None:
                ...
                pick_ban_table = form_pick_avoid_table(pick_list, comboname)
            st.session_state[f"suggest_ban_table_col_{ind}_table"] = pick_ban_table
        # remove further table
        while f"suggest_ban_table_col_{ind + 1}_table" in st.session_state:
            del st.session_state[f"suggest_ban_table_col_{ind + 1}_table"]
...
def ready_to_bp_on_click():
    ...
    if ab_cache_dict is not None:
        with open(
            os.path.join(record_folder, "depth_limit_1_warmup_cache_dict.pkl"), "wb"
        ) as f:
    ...
    st.session_state.info_placeholder = (
        "In preparation phase, we only suggest Combos for Pos 3, 4 and 5."
    ...
    if "suggest_header_placeholder" in st.session_state:
        ...
        with st.container():
            ...
            while f"suggest_ban_table_col_{col_ind_c}_table" in st.session_state:
                col_ind_c += 1
            if "bad_picks_for_each_pos" in st.session_state:
                combocols = st.columns(
                    [1.0 for _ in range(col_ind_c)] + [1.5], gap="small"
                )
            else:
            ...
            if "bad_picks_for_each_pos" in st.session_state:
                with combocols[-1]:
                    ...
                    if len(not_picked_inds) > 0:
                        ...
                        url_cols = [
                            f"Bad Pos {sugbanind + 1}" for sugbanind in not_picked_inds
                        ]
                ...
                for ally_ind in range(len(ally_hero_display_cols)):
                    with ally_hero_display_cols[ally_ind]:
                        st.selectbox(
                            ...
                            options=st.session_state["name_abbrev_dict_keys_list"],
                            format_func=lambda x: st.session_state["name_abbrev_dict"][
                                x
                            ],
                            index=None,
