...
from dota_banpick.st_cache import (
    load_lane_rate_info_dict,
    get_hero_csv_data_raw,
    get_online_image_urls,
    hero_counter_row_display_component,
    load_winlane_hero_dict,
    pos_description,
    load_today_hero_winrate_dict,
    get_hero_csv_data_filtered,
    get_image_data,
    get_pos_1_hero_list,
    get_pos_2_hero_list,
    get_pos_3_hero_list,
    get_pos_4_hero_list,
    get_pos_5_hero_list,
    get_position_colour_tags,
    get_position_default_imgspath,
    render_image_url_table,
)
...
def show_item_component(img, name, winrate, position, suggest_num=5):
    with st.expander(
        ...
        and st.session_state["if_show_counter"] in ["Win Lane", "Win Game"]
        else True,
    ):
        hero_id = st.session_state["hero_lane_rate_info_dict"][name]["url"].split("/")[
            -1
        ]
        ...
        if "if_show_counter" in st.session_state:
            if st.session_state["if_show_counter"] in ["Win Lane", "Win Game"]:
                ...
                if st.session_state["if_show_counter"] == "Win Lane":
                    with_list = list(
                        st.session_state["hero_lanewin_with_dict"][name].keys()
                    )
                    with_list = [n for n in with_list if n in with_filter_list][
                        :suggest_num
                    ]
                    versus_list = list(
                        st.session_state["hero_lanewin_versus_dict"][name].keys()
                    )
                    versus_list = [n for n in versus_list if n in versus_filter_list][
                        :suggest_num
                    ]
                    caption_str = f" Win Lane Advice"

                elif st.session_state["if_show_counter"] == "Win Game":
                    with_dict, counter_dict, bad_counter_dict = (
                        compute_with_and_counter_heroes_for_each_pos(
                            [name], suggest_num
                        )
                    )
                    if with_pos == -1:
                    ...
                    first_ele_size = (
                        suggest_num // versus_pos_len + suggest_num % versus_pos_len
                    )
                ...
                if len(with_list) == suggest_num and position != 2:
                    if st.session_state["screen_width"] < 800:
                        ...
                        render_image_url_table(dataframe_c, ["CNTR"])
                        render_image_url_table(dataframe_w, ["WITH"])
                    else:
                        dataframe = {
                            f"CNTR": get_online_image_urls(versus_list),
                            f"WITH": get_online_image_urls(with_list),
                        ...
                        st.caption(caption_str)
                        render_image_url_table(dataframe, ["CNTR", "WITH"])
                else:
                    dataframe = {
                        f"CNTR": get_online_image_urls(versus_list),
                    }
                    ...
                    dataframe = pd.DataFrame(dataframe)
                    st.caption(caption_str)
                    render_image_url_table(dataframe, ["CNTR"])
...
if __name__ == "__main__":
    ...
    st.warning(
        "这是这两三天我改进出来的Demo,功能还不够完善，我还没来得及翻译，我可以每周根据你们提的要求往这个网站里面添加或修改你们所需的功能。我也可以逐渐添加包括但不限于针对不同战队的BP推荐，各个战队视野和偏好统计等其他功能。希望可以助力CN DOTA"
    )
    ...
    st.text(
        "This is a web app for DOTA2 Ban Pick Tool, you can use it to get some drafting suggestions for DOTA2 games."
    )
...
""",
    ...
    if "show_pos_hero_ind" not in st.session_state:
        st.session_state["show_pos_hero_ind"] = (
            1  # -1 means not showing, 1 means show carry by default
        )
    ...
    if "hero_lanewin_versus_dict" not in st.session_state:
        (
            st.session_state["hero_lanewin_versus_dict"],
            st.session_state["hero_lanewin_with_dict"],
        ) = load_winlane_hero_dict()
    ...
    if "screen_width" not in st.session_state:
        st.session_state["screen_width"] = int(
            streamlit_js_eval(js_expressions="screen.width", key="SCR")
        )
...
[data-testid="column"] {
    ...
    card(
        "Start Editing Your Hero Pool",
        text=("Provide your usual positions → Edit your hero pool → Begin to banpick"),
        url="/Edit_Hero_Pool",
    ...
    st.warning(
        "If you are looking for Win Game advice, please check out the Hero Counter page."
    )
    st.radio(
        "What's your preference for hero advice?",
        ["Win Lane", "Win Game", "Hide"],
        captions=[
            "You may not win the game in the end but you can smoothly survive the early game and let others to carry you.",
            "Calm down and be patient, you can win the game!",
            "Hide for saving space.",
        ],
        key="if_show_counter",
        index=1,
    )
    ...
    for ind, col in enumerate(card_cols):
        with col:
            ...
            card_hasclick = card(
                pos_desc_text,
                text=pos_text,
                image=get_position_default_imgspath()[ind],
                styles={
                    "card": {
                        "width": "100%",
                        "height": "120px",
                ...
                key=f"pos_card_{ind}_{st.session_state.card_click_count}",
                on_click=pos_card_on_click,
            )
    ...
    if tdf is not None:
        annotated_text(
            (
                pos_description[st.session_state["show_pos_hero_ind"]].split(" ")[-1],
                f"pos {st.session_state['show_pos_hero_ind'] + 1}",
                get_position_colour_tags()[st.session_state["show_pos_hero_ind"]],
            )
        )
        ...
        winrates = [
            st.session_state["today_winrate_dict"][name]["winrate"]
        ...
        if st.session_state["screen_width"] < 800:
            if st.session_state["if_show_counter"] in ["Win Lane", "Win Game"]:
                row_display_component(
                    args_list,
                    show_cntr_image_width_mobile,
                    st.session_state["show_pos_hero_ind"] + 1,
                )
            else:
                row_display_component(
                    args_list,
                    normal_image_width_mobile,
                    st.session_state["show_pos_hero_ind"] + 1,
                )
        else:
            if st.session_state["if_show_counter"] in ["Win Lane", "Win Game"]:
                row_display_component(
                    args_list,
                    show_cntr_image_width_pc,
                    st.session_state["show_pos_hero_ind"] + 1,
                )
            else:
                row_display_component(
                    args_list,
                    normal_image_width_pc,
                    st.session_state["show_pos_hero_ind"] + 1,
                )
